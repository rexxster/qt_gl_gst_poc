...
int
GLPowerVRWidget::totalVidBuffers()
{
  int total = 0;
...
bool
GLPowerVRWidget::loadNewTexture(int vidIx)
{
  ...
  if (m_vidBufferAddressesSet.contains(false)) {
    ...
    if (!m_vidBufferAddressesSet.contains(false)) {
      ...
      for (bufPtr = m_vidBufferAddresses[currentVidIx].begin(); bufPtr != m_vidBufferAddresses[currentVidIx].end(); ++bufPtr) {
        if (ioctl(m_bcFds[currentVidIx], BCIOSET_BUFFERPHYADDR, bufPtr) != 0) {
          LOG(LOG_GL, Logger::Error, "ERROR: BCIOSET_BUFFERADDR[%d]: failed (0x%lx)", bufPtr->index, bufPtr->pa);
        }
      }
    }
    ...
    for (int currentVidIx = 0; currentVidIx < m_vidPipelines.size(); currentVidIx++) {
      ...
      for (bufPtr = m_vidBufferAddresses[currentVidIx].begin(); bufPtr != m_vidBufferAddresses[currentVidIx].end(); ++bufPtr) {
        ...
        LOG(LOG_GL, Logger::Debug1, "calling glBindTexture texId=%d", newTexId);
    ...
    LOG(LOG_GL, Logger::Debug2, "vid %d, looking up bc_buf_ptr_t index for buf pa=%lx", vidIx, currentVidBufferAddress);
    ...
    if (bufAdrFound) {
      ...
      LOG(LOG_GL, Logger::Error, "new vid buffer arrived after all expected buffers have been setup, pa=%lx", currentVidBufferAddress);
    }
  }
